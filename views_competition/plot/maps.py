...
from matplotlib import pyplot as plt, image as mpimg, colors
...
from views_competition import datautils, config, DATA_DIR, ROOT_DIR, TIMEFRAMES
...
class MapData:
    ...
    @staticmethod
    def _get_months():
        df = pd.read_parquet(os.path.join(DATA_DIR, "month.parquet"))
        df["date_str"] = df.year.astype(str) + "-" + df.month.astype(str)
        return df

    @staticmethod
    def _get_gdf_pgm() -> gpd.GeoDataFrame:
        gdf_pgm = gpd.read_file(
            os.path.join(DATA_DIR, "priogrid.geojson")
        ).set_index("pg_id")
        df_pg_c = pd.read_parquet(
            os.path.join(DATA_DIR, "skeleton_pg_c.parquet")
        )
        df_pg_c = df_pg_c.rename(columns={"country_id": "geo_country_id"})
        return gdf_pgm.join(df_pg_c)

    def __init__(self):
        """Read data from views spec."""
        self.gdf_cm = gpd.read_file(
            os.path.join(DATA_DIR, "country.geojson")
...
def plot_map(
    ...
    if country_id is not None:
        if "pg_id" in indices:
            ...
            if s_marker is not None:
                sub_marker = s_marker.index.get_level_values("pg_id").isin(
                    cpgm
                )
                s_marker = s_marker[sub_marker]
    ...
    if gdf_patch is None:
        gdf_patch = mapdata.gdf_from_series_patch(s_patch_t)
    # If gdf explicitly provided, replace s_patch (in case it's transformed).
    else:
        gdf_patch = gdf_patch.copy()
        gdf_patch = gdf_patch.join(s_patch_t, how="inner")
    ...
    if country_id:
        if neighbor_borders:
            ...
            if isinstance(country_id, List):
                sub = geom_c.index.get_level_values("country_id").isin(
                    country_id
                )
            else:
    ...
    # Plot markers on top of colour patches.
    if s_marker is not None:
        s_marker_t = s_marker.loc[t].copy() if t else s_marker.copy()
        markersize = 5 if "pg_id" in indices else 25
        if (s_marker_t < 0).any() and (s_marker_t > 0).any():  # Delta.
            gdf_marker_hi = mapdata.gdf_from_series_patch(
                s_marker_t[s_marker_t > 0]
            )
            gdf_marker_hi.centroid.plot(
                ax=ax, marker="^", markersize=markersize, color="black"
            )
            gdf_marker_lo = mapdata.gdf_from_series_patch(
                s_marker_t[s_marker_t < 0]
            )
            gdf_marker_lo.centroid.plot(
                ax=ax, marker="v", markersize=markersize, color="black"
            )
        else:  # Dummy.
            if (s_marker_t > 0).any():
                gdf_marker = mapdata.gdf_from_series_patch(
                    s_marker_t[s_marker_t > 0]
                )
    ...
    if title:
        ...
        if t:
            ax.set_title(t, fontsize=textsize + 10, pad=15)
...
def add_textbox_to_ax(
    ...
    # Add the ViEWS logo.
    path_logo_views = os.path.join(ROOT_DIR, "assets", "logo_transparent.png")
...
def make_ticks(var_scale: str) -> Dict[str, Any]:
    ...
    def make_ticks_logit() -> Tuple[List[float], List[str]]:
        ...
        for tick in ticks:
            ticks_logit.append(datautils.prob_to_logodds(tick))
...
def make_maps(df, column_sets, out_path):
    """Makes prediction maps for the submissions."""
    level = "cm" if "country_id" in df.index.names else "pgm"
    task = datautils.determine_task(df)
    size_gridborder = 0 if level == "pgm" else 0.2
    vmin, vmax = (-6, 6) if level == "cm" else (-4, 4)
    mapdata = MapData()
    for team_id, columns in column_sets[task][level].items():
        for t in TIMEFRAMES[task]:
            # A single column for task one here.
            col_list = [columns] if task == 1 else columns.values()
            for col in col_list:
                plot_map(
                    mapdata=mapdata,
                    s_patch=df[col],
                    t=t,
                    title=f"{to_datestr(t)}, {col}",
                    logodds=False,
                    run_id=f"{team_id}",
                    cmap="seismic",
                    ...
                    ymin=vmin,
                    ymax=vmax,
                    size_gridborder=size_gridborder,
                    path=os.path.join(
                        out_path, f"t{task}_{level}_{col}_{t}.png"
                    ),
                    dpi=200,
                )
    # And the ensembles (not in column_sets).
    for t in TIMEFRAMES[task]:
        for col in [col for col in df if "ensemble" in col]:
            plot_map(
                mapdata=mapdata,
                s_patch=df[col],
                t=t,
                ...
                ymax=vmax,
                size_gridborder=size_gridborder,
                path=os.path.join(out_path, f"t{task}_{level}_{col}_{t}.png"),
                dpi=200,
            )


def make_actual_maps(df, out_path):
    """Makes prediction maps for all submissions in df."""
    level = "cm" if "country_id" in df.index.names else "pgm"
    task = datautils.determine_task(df)
    size_gridborder = 0 if level == "pgm" else 0.2
    vmin, vmax = (-6, 6) if level == "cm" else (-4, 4)
    steps = [2, 3, 4, 5, 6, 7]  # Only needed for task two's shape.
    cols = (
        [config.COL_OBS_T1]
        if task == 1
        else [config.COL_OBS.format(step) for step in steps]
    )
    mapdata = MapData()
    for t in TIMEFRAMES[task]:
        for col in cols:
...
def make_error_maps(df, column_sets, out_path):
    """Makes error maps for all submissions in df."""
    level = "cm" if "country_id" in df.index.names else "pgm"
    task = datautils.determine_task(df)
    size_gridborder = 0 if level == "pgm" else 0.2
    vmin, vmax = (-6, 6) if level == "cm" else (-4, 4)
    ...
    for team_id, columns in sets.items():
        if task == 1:  # Single-month deltas for task one.
            error = df[columns] - df[config.COL_OBS_T1]
            error.name = f"error_{columns}"  # Single column under columns.
            for step, t in enumerate(TIMEFRAMES[task], 2):
                plot_map(
                    s_patch=error,
                    mapdata=mapdata,
                    t=t,
                    title=f"{to_datestr(t)}, error {team_id}, s{step}",
                    logodds=False,
                    textbox=f"Team: {team_id}\nName: error_{columns}_s{step}",
                    cmap="seismic",
                    bbox="mainland_africa",
                    ymin=vmin,
                    ymax=vmax,
                    ...
                    path=os.path.join(
                        out_path,
                        f"t{task}_{level}_error_{team_id}_s{step}_{t}.png",
                    ),
                    dpi=200,
                )
        else:  # Deltas for all months, for each stepwise column.
            for step, col in columns.items():
                error = df[col] - df[config.COL_OBS.format(step)]
                error.name = f"error_{col}"
                for t in TIMEFRAMES[task]:
                    plot_map(
                        s_patch=error,
                        mapdata=mapdata,
